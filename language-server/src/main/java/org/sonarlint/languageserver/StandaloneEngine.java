...
public class StandaloneEngine extends AbstractEngine {
  ...
  @Override
  public AnalysisResults analyze(Path baseDir, Iterable<ClientInputFile> inputFiles, Map<String, String> extraProperties, IssueListener issueListener, @Nullable LogOutput logOutput, @Nullable ProgressMonitor monitor) {
    StandaloneAnalysisConfiguration configuration = new StandaloneAnalysisConfiguration(baseDir, baseDir.resolve(".sonarlint"),
        inputFiles,
        extraProperties);
    
      debug("Standalone Analysis triggered on " + baseDir + " with configuration: \n" + configuration.toString());
