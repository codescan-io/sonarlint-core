...
public class ConnectedEngine extends AbstractEngine {
  ...
  private static ServerConfiguration getServerConfiguration(SonarQubeServer server) {
    ...
    String organization = server.organization();
    if ( !StringUtils.isEmpty(organization) )
      serverConfigBuilder.organizationKey(organization);
  ...
  @Override
  public AnalysisResults analyze(Path baseDir, Iterable<ClientInputFile> inputFiles,
    ...
    extraProperties.put("sonar.host.url", server.url());
    extraProperties.put("sonar.organization", server.organization());
    String token = server.token();
    if ( token != null ) {
      extraProperties.put("sonar.login", token);
    }else {
      extraProperties.put("sonar.login", server.login());
      extraProperties.put("sonar.password", server.password());
    }
    ...
    ConnectedAnalysisConfiguration configuration  = new ConnectedAnalysisConfiguration(workspaceProjectKey, baseDir, baseDir.resolve(".sonarlint"),
        inputFiles,
        extraProperties);
    debug("Connected Analysis triggered on " + baseDir + " with configuration: \n" + configuration.toString());
    
    
    return engine.analyze( (ConnectedAnalysisConfiguration) configuration, issueListener, logOutput, monitor);
  }
