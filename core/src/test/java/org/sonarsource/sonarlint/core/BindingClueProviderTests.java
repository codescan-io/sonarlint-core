...
class BindingClueProviderTests {
  ...
  void should_detect_sonar_scanner_for_sonarcloud_based_on_url() throws InterruptedException {
    ...
    when(connectionRepository.getConnectionById(SC_CONNECTION_ID_1)).thenReturn(new SonarCloudConnectionConfiguration(SC_CONNECTION_ID_1, "http://server2", MY_ORG_1, true));
    when(connectionRepository.getConnectionById(SC_CONNECTION_ID_2)).thenReturn(new SonarCloudConnectionConfiguration(SC_CONNECTION_ID_2, "http://server2", MY_ORG_2, true));
